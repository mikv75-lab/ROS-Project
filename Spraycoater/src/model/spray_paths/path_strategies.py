...
class MeanderPlanePath(BasePath):
    ...
    def build_points(self, *, step_mm: float, max_points: int) -> np.ndarray:
        """
        Meander (2D) im lokalen Frame:
        - Bahnen sind horizontale Linien (y konstant)
        - 180°-Wende wird als reiner Polyline-U-Turn erzeugt:
            ... -> (x_end, y_i) -> (x_end, y_{i+1}) -> nächste Bahn
          (damit entsteht automatisch "2x 90°")
        - Falls zwei benachbarte Bahnen am Wendepunkt unterschiedlich lang sind,
          wird die kürzere am Wendepunkt verlängert (Turn-End Equalization).
        """
        ...
        # Grid generieren
        lines_lr: List[Tuple[Tuple[float, float], Tuple[float, float]]] = []  # ((xL,y),(xR,y)) mit xL<=xR
        ...
        if is_circle:
            ...
            for y in ys:
                if abs(y) < R:
                    span = math.sqrt(max(0.0, R * R - y * y))
                    xL = -span - extend_x
                    xR = +span + extend_x
                    lines_lr.append(((xL, float(y)), (xR, float(y))))
        else:
            ...
            ys = np.linspace(-(n_lines - 1) * pitch / 2, (n_lines - 1) * pitch / 2, n_lines)
            for y in ys:
                lines_lr.append(((-hx, float(y)), (hx, float(y))))

        if not lines_lr:
            return np.zeros((0, 3), float)

        # Start-Corner Logic (Y-Reihenfolge + initiale X-Richtung)
        start_c = str(p.get("start", "auto")).lower()
        if start_c in ("minx_maxy", "maxx_maxy"):
            lines_lr = lines_lr[::-1]  # von oben nach unten
        rev_x = start_c in ("maxx_miny", "maxx_maxy")  # True => start rechts, nach links

        # ---------------------------
        # Turn-End Equalization
        # ---------------------------
        equalize = bool(p.get("equalize_turn_ends", True))
        if equalize and len(lines_lr) >= 2:
            xL = np.array([a[0] for a, _ in lines_lr], dtype=float)
            yv = np.array([a[1] for a, _ in lines_lr], dtype=float)
            xR = np.array([b[0] for _, b in lines_lr], dtype=float)

            r = rev_x
            for i in range(len(lines_lr) - 1):
                if not r:
                    # Bahn i läuft L->R, Wende rechts: xR gleichziehen
                    target = max(xR[i], xR[i + 1])
                    xR[i] = target
                    xR[i + 1] = target
                else:
                    # Bahn i läuft R->L, Wende links: xL gleichziehen
                    target = min(xL[i], xL[i + 1])
                    xL[i] = target
                    xL[i + 1] = target
                r = not r

            lines_lr = [((float(xL[i]), float(yv[i])), (float(xR[i]), float(yv[i]))) for i in range(len(lines_lr))]

        # ---------------------------
        # Meander verbinden (reine 90°-Ecken)
        # ---------------------------
        pts_out: List[List[float]] = []

        r = rev_x
        for i, (pL, pR) in enumerate(lines_lr):
            (xL_i, y_i) = pL
            (xR_i, _) = pR
            ...
            pts_line = np.column_stack([xs, ys])

            if i > 0:
                pts_out.extend(pts_line[1:].tolist())
            else:
                pts_out.extend(pts_line.tolist())

            # U-Turn zum nächsten Segment: nur vertikal auf nächstes y
            if i < len(lines_lr) - 1:
                y_next = lines_lr[i + 1][0][1]
                pts_out.append([end[0], float(y_next)])

            r = not r

        P = np.array(pts_out, dtype=float)
        if P.shape[0] < 2:
            return np.zeros((0, 3), float)

        # Optionales Fillet (default: aus, damit 180°-Wenden exakt bleiben)
        cr = float(p.get("corner_radius_mm", 0.0))
        if cr > 0:
            try:
                P = self._fillet_polyline_2d(P, radius=cr, step=step)
            except Exception:
                pass
    ...
    def build_points(self, *, step_mm: float, max_points: int) -> np.ndarray:
        ...
        for k in range(max(1, int(p.get("loops", 1)))):
            ...
            if area.get("shape") == "circle":
                ...
                if r > 0:
                    polys.append(self._polyline_circle(cx, cy, r, step))
            else:
                sx, sy = area.get("size_mm", [100.0, 100.0])
                hx, hy = 0.5 * float(sx) - off, 0.5 * float(sy) - off
                if hx > 0 and hy > 0:
                    polys.append(
                        self._polyline_rounded_rect(
                            cx,
                            cy,
                            hx,
                            hy,
                            min(float(p.get("corner_blend_mm", 0)), hx, hy),
                            step,
                        )
                    )
...
class _PolyhelixGeneric(BasePath):
    ...
    def build(p: Dict, *, step_mm: float, max_points: int, is_cube: bool) -> np.ndarray:
        ...
        # 1) Base Outline 2D
        if is_cube:
            base = BasePath._polyline_rounded_rect(
                0,
                0,
                0.5 * edge + stand,
                0.5 * edge + stand,
                float(p.get("corner_roll_radius_mm", 8)),
                step,
            )[:, :2]
        else:
        ...
        P = np.array(pts3, dtype=float)
        return BasePath._decimate(
            BasePath._trim_by_arclength(P, float(p.get("start_offset_mm", 0)), float(p.get("end_offset_mm", 0))),
            int(max_points),
        )
