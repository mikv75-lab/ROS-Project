...
class RecipeTab(QWidget):
    ...
    def _render_preview(self, model: object):
        try:
            ...
            # 4) Spray-Visualisierung (+2D-Daten)
            mask_poly = None
            tcp_for_2d = None
            try:
                if smesh is not None:
                    paths_by_side = dict(model.get("paths_by_side") or {}) if isinstance(model, dict) \
                                    else dict(getattr(model, "paths_by_side", {}) or {})
                    if not paths_by_side:
                        _LOG.warning("Recipe hat keine paths_by_side – zeige nur Mount/Substrat.")
                        self._finalize_view_and_render()
                        # 2D-Szene leeren/anpassen
                    ...
                    try:
                        ...
                        self.previewPanel.add_mesh(
                            mask_poly, color="royalblue", opacity=1.0, lighting=False, layer="mask"
                        )
                    except Exception:
                    ...
                    # Misses (bei vertikalem Fall bis zmin)
                    rays_miss = pv.PolyData()
                    if valid is not None:
                        miss = ~valid
                        if np.any(miss):
                            Pmiss_start = Pstart[miss]
                            ray_dir = np.array([0.0, 0.0, -1.0])  # Richtung ist durch side implizit in cast_rays_for_side
                            if abs(ray_dir[2]) > 1e-6:
                                ...
                                Pmiss_end = Pmiss_start + ray_dir * 1000.0
                    ...
                    if getattr(rays_hit, "n_lines", 0):
                        self.previewPanel.add_mesh(
                            rays_hit, color="#85C1E9", line_width=1.5, lighting=False, layer="rays_hit"
                        )
                    if getattr(rays_miss, "n_lines", 0):
                        self.previewPanel.add_mesh(
                            rays_miss, color="#e74c3c", line_width=1.0, lighting=False, layer="rays_miss"
                        )

                    # TCP/Pfad/Normalen/Frames
                    tcp_for_2d = None
                    if rc is not None and hasattr(rc, "tcp_mm") and rc.tcp_mm is not None:
                        ...
                        if tcp is not None and len(tcp):
                            ...
                            self.previewPanel.add_path_polyline(tcp, color="#e74c3c", line_width=2.0,
                                                                 as_tube=False, layer="path")
                            self.previewPanel.add_path_markers(tcp, step=max(1, len(tcp) // 50),
                                                               color="#c0392b", layer="path_markers")
                            if valid is not None and np.any(valid):
                                hits = rc.hit_mm[valid]
                                norms = rc.normal[valid]
                                try:
                                    self.previewPanel.show_normals_from_hits(
                                        hits, norms, layer="normals", length_mm=8.0,
                                        color="#27ae60", line_width=1.0
                                    )
                                except Exception:
                            ...
                            try:
                                ...
                                if z_dirs is not None and len(z_dirs):
                                    self.previewPanel.show_frames_at(
                                        origins=tcp, z_dirs=z_dirs, scale_mm=1.0,
                                        line_width=None, layer="frames"
                                    )
                            except Exception:
            ...
            # 2D-Szene updaten (Substrat + Pfad + Maske)
            try:
                self.previewPanel.update_2d_scene(
