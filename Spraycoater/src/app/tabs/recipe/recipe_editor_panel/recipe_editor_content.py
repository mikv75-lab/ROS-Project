...
from PyQt6.QtWidgets import (
    ...
    QScrollArea, QSpacerItem, QSizePolicy, QTabWidget, QLineEdit, QLabel
)
...
class RecipeEditorContent(QWidget):
    ...
    Layout:
      VBox
        ...
        └─ QTabWidget (jede Side = eigener Tab mit eigener ScrollArea)
             └─ ScrollArea
                 └─ VBox
                     ├─ SidePathEditor   <-- KEINE zusätzliche GroupBox
                     └─ Spacer (Expanding)
    """
    def __init__(self, *, ctx=None, store: RecipeStore, parent: Optional[Widget] = None):
    ...
    def _build_ui(self) -> None:
        ...
        self.sideTabs.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
    ...
    def _rebuild_globals_from_schema(self) -> None:
        ...
        for key in keys:
            ...
            if t == "boolean":
                ...
                # unbekannter Typ -> statischer Text
                w = QLabel(f"(unsupported type: {t})")
                self.globals_form.addRow(label, w)

            # Default setzen, falls vorhanden
            if "default" in spec:
                dv = spec["default"]
        ...
        # technische Felder hinten dran (falls im Schema nicht enthalten)
        # sample_step_mm, max_points, max_angle_deg sind i.d.R. schon drin.
        self.globals_form.addRow(_hline())

    # ------------------- Defaults (nur Formular-Reset) -------------------
    def apply_defaults(self) -> None:
        # nutzt gesetzte Defaults aus _rebuild_globals_from_schema()
    ...
    def apply_recipe_model(self, model: Recipe, rec_def: Dict[str, Any]) -> None:
        ...
        def _set_if_present(combo: QComboBox, value: Optional[str]) -> None:
            ...
            combo.setCurrentIndex(idx if idx >= 0 else 0 if combo.count() > 0 else -1)
        ...
        # Sides → Tabs (pro Tab eigene ScrollArea) – KEINE GroupBox um den Editor
        sides: Dict[str, Any] = _coalesce_sides_from_rec_def(rec_def)

        for side_name in sides.keys():
            # Tabcontent mit Scroll
            scroll = QScrollArea()
            ...
            # Allowed + Default (streng aus Rezept; Store liefert Runtime mit schemas)
            ...
            editor._side_cfg = dict(side_runtime)  # damit set_allowed_types Pages aus schemas bauen kann
            ...
            # _side_cfg mitschicken, damit apply_default_path sie kennt
            ...
            self.sideTabs.addTab(scroll, side_name)
