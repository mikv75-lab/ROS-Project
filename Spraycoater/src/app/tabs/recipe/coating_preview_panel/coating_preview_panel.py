...
class CoatingPreviewPanel(QWidget):
    ...
    Layout:
      Info
      HBox:
        - Left:  Views2D + Matplotlib (Expanding) + Spacer
        - Right: Views3D + Overlays + PyVistaHost (Expanding)

    Info-Quelle:
      - Recipe.compile_poses(...) schreibt Basiswerte in recipe.info
      - Dieses Panel ergänzt nur Laufzeitwerte:
    ...
    interactorReady = pyqtSignal()  # Interactor ist verfügbar
    ...
    def __init__(self, *, ctx, store=None, parent: Optional[QWidget] = None):
        ...
        # Matplotlib Canvas (erst erzeugen, dann Controls davor)
        ...
        try:
            ...
            if tb is not None:
                self._mat2d_toolbar = tb  # Referenz halten
        ...
        # 2D Controls (nutzen _mat2d.refresh)
    ...
    def handle_update_preview(self, model: Recipe) -> None:
        ...
        # 3) compile_poses -> compiled + path_xyz (Basis-Info wird von Recipe selbst gepflegt)
        stand_off = 10.0
        try:
        ...
        compiled: Any = None
        try:
            if callable(getattr(model, "compile_poses", None)):
        ...
        # 7) Info ergänzen (nur Runtime-Felder) und an InfoBox geben
        try:
            self._update_recipe_info_with_runtime(scene=scene, model=model)
    ...
    def _update_recipe_info_with_runtime(self, *, scene, model: Recipe) -> None:
        """
        Ergänzt model.info um reine Runtime-Werte:
          - eta_s
          - medium_ml
          - mesh_tris
          - mesh_bounds (L,B,H in mm)

        Keine Rekonstruktion von Basiswerten (points/length) aus anderen Quellen.
        """
        ...
        speed_mm_s = float(params.get("speed_mm_s", 0.0)) if isinstance(params, dict) else 0.0
        flow_ml_min = float(params.get("flow_ml_min", 0.0)) if isinstance(params, dict) else 0.0

        length_mm = float(info.get("length_mm") or info.get("total_length_mm") or 0.0)

        eta_s: Optional[float]
        if length_mm > 0.0 and speed_mm_s > 1e-9:
            eta_s = length_mm / speed_mm_s
        else:
            eta_s = None
        ...
        # Mesh-Infos
        info["mesh_tris"] = getattr(scene, "mesh_tris", None)
        ...
        if b is not None:
            try:
                ...
                info["mesh_bounds"] = (float(xmax - xmin), float(ymax - ymin), float(zmax - zmin))
            except Exception:
                pass
