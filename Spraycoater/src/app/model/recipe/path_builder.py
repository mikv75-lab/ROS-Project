...
class PathData:
    ...
    - meta: freie Metadaten zum Pfad (Quelle, Parameter, etc.)
    """
...
class PathBuilder:
    ...
    Alternativ kann ein Pfad direkt über `points_mm` oder `polyline_mm`
    geliefert werden (ohne Type-Auswertung).

    Hinweis:
    - Diese Klasse arbeitet bewusst strikt. Wenn Pflichtparameter fehlen oder
      ein Typ unbekannt ist, wird eine Exception geworfen.
    """
    ...
    def from_side(
        ...
        *,
        ...
        # Aktuell keine Geometrie-Änderung (kein Pre/Retreat)
        pd = PathBuilder._postprocess(pd)

        # Globals in meta übernehmen
    ...
    def _decimate(P: np.ndarray, max_points: int) -> np.ndarray:
        """Downsampling über Stride, um die Punktzahl zu begrenzen."""
    ...
    def _as_points_mm(pts: Any, max_points: int) -> np.ndarray:
        """Konvertiert Punkte nach (N,3) float und begrenzt die Punktzahl."""
    ...
    def _postprocess(pd: PathData) -> PathData:
        ...
        Aktuell: keine Änderungen an der Geometrie.
        """
