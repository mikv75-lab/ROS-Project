...
# WICHTIG: Import der optimierten Threading-Funktion
...
def _call_cast_rays_for_side_robust(*args, **kwargs):
    # WICHTIG: Hier wird die optimierte cast_rays_for_side aus raycast_projector aufgerufen
    ...
    hit_poly = polys[0] if len(polys) > 0 else pv.PolyData()
    miss_poly = polys[1] if len(polys) > 1 else pv.PolyData()
    tcp_poly = polys[2] if len(polys) > 2 else pv.PolyData()
    return rc, hit_poly, miss_poly, tcp_poly

def _is_empty_poly(mesh) -> bool:
    try:
        if mesh is None: return True
        if isinstance(mesh, pv.DataSet): return int(getattr(mesh, "n_points", 0)) <= 0
...
class CoatingPreviewPanel(QWidget):
    ...
    def __init__(self, *, ctx, store: RecipeStore, parent: Optional[QWidget] = None):
        ...
        self._vis = {
            "path": True, "mask": True, "hits": True, "misses": True, "normals": True, "frames": True,
        }
        self._logged_pathbuilder_origin = False
        ...
        try:
            ...
            if tb is not None: vleft.addWidget(tb, 0)
        except Exception: _LOG.exception("Matplot2D toolbar creation failed")
        ...
        self.grpOverlays = OverlaysGroupBox(
            ...
            layers={
                "ground": "ground", "mount": "mount", "substrate": "substrate",
                "path": "path", "path_mrk": "path_markers", "mask": "mask", "mask_mrk": "mask_markers",
                "rays_hit": "rays_hit", "rays_miss": "rays_miss", "normals": "normals",
                "frames_x": "frames_x", "frames_y": "frames_y", "frames_z": "frames_z",
    ...
    def handle_update_preview(self, model: Recipe) -> None:
        ...
        max_angle_deg = float(globals_params.get("max_angle_deg", 90.0))
        ...
        built = PathBuilder.from_recipe_paths(
            recipe=model, sides=sides, globals_params=globals_params,
            sample_step_mm=sample_step_mm, max_points=max_points,
        )

        for lyr in ("path", "path_markers", "mask", "mask_markers", "rays_hit", "rays_miss", "normals", "frames_x", "frames_y", "frames_z", "planned", "executed"):
        ...
        self.grpOverlays.overlays.render_compiled(
            ...
            planned_tcp=planned_tcp, executed_tcp=executed_tcp,
            visibility=vis,
            mask_lift_mm=50.0, default_stand_off_mm=stand_off_mm,
            sides=pbs
        )
        ...
        for side_id, pdata in built:
            ...
            if side not in _ALLOWED_SIDES: continue

            pts = getattr(pdata, "points_mm", None)
            if pts is None: pts = getattr(pdata, "points", None)
            if pts is None: continue

            P_local = np.asarray(pts, dtype=float).reshape(-1, 3)
            if P_local.size == 0: continue
            ...
            # Check if Helix/Cylinder
            is_helix = side in ("helix", "polyhelix") or "spiral.cylinder" in source

            if is_helix:
                P_start = _embed_3d_world_centered(P_local, substrate_mesh.bounds)
                if side == "helix" and (not source): source = "helix"
                # ✅ Korrektur: Wir lassen side_for_rays auf 'helix', 
                # damit raycast_projector nicht in den 'base_dir'-Fallback (top) läuft,
                # falls 'source' leer sein sollte. 
                # (Aber PathBuilder setzt source='path.spiral.cylinder' korrekt).
                side_for_rays = "helix" 
            else:
            ...
            try:
                # WICHTIG: Aufruf der Threading-Funktion
            ...
            # --- Z-Achse (Normale) mit Max-Angle-Limit ---
            raw_nrm = _normalize(np.asarray(rc.normal, float).reshape(-1, 3)[valid])
            Z = np.zeros_like(raw_nrm)
            
            up_vec = np.array([0.0, 0.0, 1.0]) 
            for i in range(len(raw_nrm)):
                # Bei Helix (Seitenflächen) ist die "ideale" Normale waagerecht. 
                # Das Limit bezieht sich auf die Abweichung von Welt-Z.
                # Standard: Limit Abweichung von "Oben".
            ...
            # --- ORIENTIERUNG (Fixed Orientation / Tangential) ---
            
            if is_helix:
                # Helix/Zylinder Modus:
                # Z = Radial nach außen (limitiert)
                # Up = Welt-Z
                # X = Cross(Z, Up) -> Tangential am Umfang (waagerecht)
                # Y = Cross(Z, X) -> Vertikal am Zylinder
                
                X_helix = np.cross(Z, up_vec)
                X_helix = _normalize(X_helix)
                
                # Fallback am Pol (sollte bei Helix/Zylinder-Wand nicht passieren)
                bad = np.linalg.norm(X_helix, axis=1) < 1e-3
                if np.any(bad):
                    X_helix[bad] = np.array([1.0, 0.0, 0.0]) 
                ...
                # Top/Flat Modus:
                # Z = Nach oben (limitiert)
                # X = Welt-X projiziert (Fixed Orientation)
                
                sid = str(side_id).lower()
                if sid in ("left", "right"): base_ref = np.array([0.0, 1.0, 0.0], float)
                else: base_ref = np.array([1.0, 0.0, 0.0], float)

                ref_X = np.tile(base_ref, (len(Z), 1))
                ...
                if np.any(bad):
                    fallback = np.tile(np.array([0.0, 1.0, 0.0], float), (len(X), 1))
            ...
            if vis.get("frames", False):
                ...
                O2 = tcp_world[::sstep]; X2 = X[::sstep]; Y2 = Y[::sstep]; Z2 = Z[::sstep]
                sx = 12.0
            ...
            try:
                ...
                for i in range(len(tcp_local)):
                    poses_quat.append({
                        "x": float(tcp_local[i, 0]), "y": float(tcp_local[i, 1]), "z": float(tcp_local[i, 2]),
                        "qx": float(quats[i, 0]), "qy": float(quats[i, 1]), "qz": float(quats[i, 2]), "qw": float(quats[i, 3]),
                    })
                normals_xyz = [{"x": float(nx), "y": float(ny), "z": float(nz)} for (nx, ny, nz) in Z.reshape(-1, 3)]

                base: Dict[str, Any] = {}
                if isinstance(getattr(model, "draft", None), Draft):
                    try: base = dict(model.draft.to_yaml_dict() or {}) 
                    except Exception: base = {}
                
                base.setdefault("version", 1)
                base.setdefault("sides", {})
                if not isinstance(base.get("sides"), dict): base["sides"] = {}

                base["sides"][str(side)] = { "poses_quat": poses_quat, "normals_xyz": normals_xyz }
        ...
        path_xyz = None
        if tcp_all_world:
            try: path_xyz = np.vstack(tcp_all_world)
            except Exception: path_xyz = None

        try: self.update_2d_scene(substrate_mesh=scene.substrate_mesh, path_xyz=path_xyz)
        except Exception: _LOG.exception("update_2d_scene failed")

        try:
            info = { "points": int(0 if path_xyz is None else len(path_xyz)), "tris": scene.mesh_tris, "bounds": scene.bounds }
        ...
        try:
            if cam_snap: self.restore_camera(cam_snap)
        except Exception: pass
    ...
    def _push_initial_visibility(self) -> None:
        try: self.scene.update_current_views_once(refresh_2d=self._mat2d.refresh)
        except Exception: _LOG.exception("initial visibility push failed")

    def render(self) -> None:
        ia = self._interactor
        try:
            if ia is not None and hasattr(ia, "render"): ia.render()
        except Exception: _LOG.exception("render failed")

    def snapshot_camera(self) -> Optional[Dict[str, Any]]:
        ia = self._interactor
        if ia is None or not hasattr(ia, "camera"): return None
        cam = ia.camera
        try:
            return {
                "position": tuple(cam.position), "focal_point": tuple(cam.focal_point),
                "view_up": tuple(cam.up), "clipping_range": tuple(cam.clipping_range),
            }
    ...
    def restore_camera(self, snap: Optional[Dict[str, Any]]) -> None:
        if not snap: return
        ia = self._interactor
        if ia is None or not hasattr(ia, "camera"): return
        try:
            cam = ia.camera
            ...
            if cr: cam.clipping_range = cr
        except Exception: _LOG.exception("restore_camera failed")

    def set_bounds(self, bounds: Bounds) -> None:
        self._bounds = tuple(map(float, bounds))
        try: self._mat2d.set_bounds(self._bounds)
        except Exception: _LOG.exception("set_bounds -> 2D failed")
    ...
    def _switch_2d_plane(self, plane: str) -> None:
        try:
            ...
            self._mat2d.refresh()
        except Exception: _LOG.exception("2D plane switch failed: %s", plane)
    ...
    def clear(self) -> None: self.scene.clear()
    def add_mesh(self, mesh, **kwargs) -> None: self.scene.add_mesh(mesh, **kwargs)
    def clear_layer(self, layer: str) -> None: self.scene.clear_layer(layer)
    def add_path_polyline(self, *a, **kw): return self.scene.add_path_polyline(*a, **kw)
    ...
    def show_poly(self, poly, *, layer: str, color: str = "royalblue", line_width: float = 2.0, lighting: bool = False) -> None:
        ...
        except Exception: _LOG.exception("show_poly() failed")

    def show_frames_at(self, **_kwargs) -> None: return