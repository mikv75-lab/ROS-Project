...
class BaseProcessStatemachine(QtCore.QObject):
    ...
    def _segment_exists(self, seg_name: str) -> bool:
        """Klasse prüft, ob das Segment im Rezept definiert ist."""
        return True
    ...
    def _on_motion_result(self, result: str) -> None:
        if not self._machine or not self._machine.isRunning() or self._stop_requested:
            return
        res = (result or "").strip()
        if not res or self._wait_active:
            return
    ...
    def _jt_from_any(cls, obj: Any, *, _depth: int = 0, _max_depth: int = 8) -> Optional[Dict[str, Any]]:
        ...
        # 1. Check for JointTrajectory structure directly
        try:
        ...
        # 2. Nested joint_trajectory attribute (MoveIt Plan)
        try:
        ...
        # 3. Dict Recursion (Restored lines)
    ...
    def _on_finished(self) -> None:
        """Erzeugt die finalen JTBySegment Payloads und beendet den Worker."""
        ...
        # Rückgabe des serialisierbaren Payloads an den ProcessThread
        self.notifyFinished.emit(self._rr.to_process_payload())
        self._cleanup()

    def _jt_by_segment_yaml(self, *, which: str) -> Dict[str, Any]:
        """Konvertiert die internen Daten in das YAML v1 Format von JTBySegment."""
    ...
    def _on_enter_segment(self, seg_name: str) -> None: pass
    def _on_segment_ok(self, seg_name: str) -> None: self._snapshot_trajs_for_segment(seg_name)
    def _should_transition_on_ok(self, seg_name: str, result: str) -> bool: return True
    ...
    def _on_planned_traj_changed(self, obj): self._last_planned_any = obj
    @QtCore.pyqtSlot(object)
    def _on_executed_traj_changed(self, obj): self._last_executed_any = obj