...
def generate_launch_description():
    ...
    def launch_setup(context):
        ...
        # 1) static TF (muss garantiert zuerst laufen)
        static_tf = Node(
            package='tf2_ros', executable='static_transform_publisher', name='tf_world_to_mecamount',
            arguments=[
                "--x", str(world_xyz[0]), "--y", str(world_xyz[1]), "--z", str(world_xyz[2]),
                "--qx", str(qx), "--qy", str(qy), "--qz", str(qz), "--qw", str(qw),
                "--frame-id", "world", "--child-frame-id", parent_frame
            ],
            output="screen",
        )
        ...
        # 3) tool_manager NACH poses_manager (startet Roboter/MoveIt je nach Tool)
        tool_mgr = Node(
        ...
        # 4) Nach Start des tool_manager: Cage → Workspace → Workpiece
        delayed = []
        if cg_enable:
            cage_spawner = Node(
            ...
            delayed.append(TimerAction(period=2.0, actions=[cage_spawner]))
        ...
        if wm_enable:
            ...
            delayed.append(TimerAction(period=4.0, actions=[workspace_spawner]))
        ...
        delayed.append(TimerAction(period=6.0, actions=[workpiece_mgr]))

        chain3 = RegisterEventHandler(OnProcessStart(target_action=tool_mgr, on_start=delayed))

        return env + [static_tf, chain1, chain2, chain3]
